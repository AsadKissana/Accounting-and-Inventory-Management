    ...
    <div className="max-w-7xl mx-auto">
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-gray-800 mb-4">Sales Orders</h3>
          <div className="space-y-2">
            {orders.map(so => (
              <button
                key={so.id}
                onClick={() => setSelectedSO(so)}
                className={`w-full text-left p-3 rounded-lg border transition-colors ${selectedSO?.id === so.id
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-gray-900">{so.orderNo}</p>
                    <p className="text-sm text-gray-500">{so.customer}</p>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded ${so.status === 'Pending' ? 'bg-yellow-100 text-yellow-800' :
                ...
                <p className="text-sm text-gray-500 mt-1">${so.total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
              </button>
            ))}
            {orders.length === 0 && (
              <p className="text-gray-500 text-center py-4">No sales orders found</p>
            )}
          </div>
        </div>
          ...
          {selectedSO ? (
            <>
              <div className="flex justify-between items-center mb-6 print:hidden">
                <h3 className="text-gray-800">Sales Order Preview</h3>
                <button
                  onClick={handlePrint}
              ...
              <div className="border border-gray-300 p-8 bg-white" id="printable">
                ...
                <div className="text-center mb-6">
                  ...
                  <h3 className="text-gray-800">Sales Order</h3>
                </div>

                <div className="grid grid-cols-2 gap-8 mb-6">
                  <div>
                    <h4 className="text-gray-700 mb-2">Customer:</h4>
                    <p className="text-gray-900">{selectedSO.customer}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-gray-600">SO No: <span className="text-gray-900">{selectedSO.orderNo}</span></p>
                    <p className="text-gray-600">Date: <span className="text-gray-900">{selectedSO.date}</span></p>
                    <p className="text-gray-600">Delivery Date: <span className="text-gray-900">{selectedSO.deliveryDate}</span></p>
                    <p className="text-gray-600">Terms: <span className="text-gray-900">{selectedSO.terms}</span></p>
                  </div>
                </div>
                ...
                <table className="w-full mb-6">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {selectedSO.lines.map((line, index) => (
                      <tr key={index}>
                  ...
                  <tfoot className="border-t-2 border-gray-300">
                    <tr>
                      ...
                      <td className="px-4 py-3 text-right text-gray-900">
                        ${selectedSO.total.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                      </td>
                    </tr>
                  </tfoot>
                </table>
                ...
                <div className="mt-8 pt-6 border-t border-gray-300">
                  ...
                  <ul className="text-sm text-gray-600 space-y-1">
                    <li>• Payment terms: {selectedSO.terms}</li>
                    <li>• Delivery must be made by {selectedSO.deliveryDate}</li>
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500">
              ...
              <p>Select a sales order to preview</p>
            </div>
          )}
        </div>
      </div>
